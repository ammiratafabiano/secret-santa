...
async def report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        n_users = len(storage.load_users())
        n_groups = len(storage.load_groups())
        n_completed_groups = storage.load_report().n_completed_groups
...
def add_group_completed_report(context):
    report_data = storage.load_report()

    try:
        if report_data.n_completed_groups:
            report_data.n_completed_groups += 1
        else:
            report_data.n_completed_groups = 1
